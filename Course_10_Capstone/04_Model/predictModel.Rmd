...
tri_fun <- function(w1, w2, n = 5){
  ...
  pwords <-
    ...
    filter(word1 == w1) %>%
    filter(word2 == w2) %>%
    arrange(desc(triProb))
  
  if (any(is.na(pwords)))
    
    return(bi_fun(w2, n)) # backoff to bi-grams prediction
  
  if (nrow(pwords) > n)
    
    return(pwords %>% top(n) %>% select(word3)) # propose 5 options
...
bi_fun <- function(w1, n = 5){
    ...
    if (nrow(pwords) > n)
          ...
          return(pwords %>% top(n) %>% select(word2))
...
If we couldn’t even find the corresponding bi-gram, we randomly get a word from unigrams with high probability. This is the last resort for n-grams that are not found in the dataset.

```{r PREDICT UNIGRAM}

uni_fun <- function(n = 5){
  
    return(sample(unigram[ , word], size = n))
...
getWords <- function(str){
    ...
    words <- tri_fun(tokens[1], tokens[2], 5)
